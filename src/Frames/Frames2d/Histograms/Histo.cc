...
void
Histo::PreInitialise( const ConfigurationTable* configTable )
{
...
void
Histo::Render2d( RWWrapper& windowApp,
...
void
Histo::DrawTickLabel( double value,
                      bool xAxis )
{
...
void
Histo::CalculateHistogram( const RenderState& renderState )
{
...
int
Histo::CalculateBin( double value )
{
  if( value <= fXDomain.first )
    ...
    return static_cast<int>( ( value - fXDomain.first ) / ( fXDomain.second - fXDomain.first ) * fValues.size() ) + 2;
}

double
Histo::ScaleY( double value )
{
  if( true && value > 0.0 && fYRange.second > 1.0 )
