...
def clear_include():
    ...
    files.extend( glob.glob( rids_dir + "/*" ) )
    for f in files:
        os.remove(f)    
...
def copy_to_include(src_header, src_dir, rids_directory):
    global inc_dir, rids_dir
    if rids_directory:
        inc_header = rids_dir + src_header.replace( src_dir, "" )
    else:
...
def src_module(env, module_name, rids_directory = False):
    ...
    for h in headers:
        copy_to_include(h, src_dir, rids_directory)
